...
class StreakService {
  ...
  static Future<StreakInfo> fetchStreak() async {
    ...
    if (body is Map<String, dynamic>) {
      ...
      final levelName = body['levelName'] is String ? body['levelName'] : (level is String ? level : null);
      return StreakInfo(
        ...
        totalSessions: body['totalSessions'] is int ? body['totalSessions'] : (body['totalSessions'] is num ? (body['totalSessions'] as num).toInt() : 0),
        sessionsToNextLevel: body['sessionsToNextLevel'] is int ? body['sessionsToNextLevel'] : (body['sessionsToNextLevel'] is num ? (body['sessionsToNextLevel'] as num).toInt() : 0),
        levelName: levelName,
        nextLevelName: body['nextLevelName'] is String ? body['nextLevelName'] : null,
      );
    }
    return StreakInfo(currentStreak: 0, life: 3, level: 1, totalSessions: 0, sessionsToNextLevel: 6);
  }
  ...
  static Future<SessionResult> completeSession({
    ...
    if (body is Map<String, dynamic>) {
      ...
      final totalXpInLevel = body['totalXpInLevel'] is int ? body['totalXpInLevel'] as int : (body['totalXpInLevel'] is num ? (body['totalXpInLevel'] as num).toInt() : 0);
  ...
  static int parseLevel(Object? value) {
    ...
    if (value is String) {
      switch (value.toUpperCase()) {
        ...
        default: return int.tryParse(value) ?? 1;
      }
    }
    return 1;
  }
